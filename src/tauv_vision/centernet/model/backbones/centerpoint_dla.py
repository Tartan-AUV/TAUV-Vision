...
from tauv_vision.centernet.model.centernet import get_head_channels, Prediction
...
class BasicBlock(nn.Module):
    ...
    def forward(self, x, residual=None):
        ...
        residual = pad_to_match(residual, out.shape)
        out += residual
        out = self.relu(out)
...
class DeformConv(nn.Module):
    def __init__(self, chi, cho):
        ...
        self.conv = torchvision.ops.DeformConv2d(chi, cho, kernel_size=kernel_size, stride=stride, padding=padding, dilation=1, groups=1)

    def forward(self, x):
        x = self.conv(x, self.offset(x), F.sigmoid(self.mask(x)))
        x = self.actf(x)
        return x
...
def pad_to_match(feature: torch.Tensor, shape: Tuple[int, ...]) -> torch.Tensor:
    ...
    padded_feature = F.pad(feature, (pad_left, pad_right, pad_above, pad_below))
...
class IDAUp(nn.Module):
    ...
    def forward(self, layers, startp, endp):
        for i in range(startp + 1, endp):
            ...
            layers[i] = node(pad_to_match(layers[i], layers[i - 1].shape) + layers[i - 1])
...
class CenterpointDLA34(nn.Module):
    ...
    def __init__(self, object_config):
        ...
        head_channels = get_head_channels(object_config)
